...
export function processLinearData({ completed, inProgress, blockedOpen = [] }, now = new Date()) {
  const { curStart, now: endNow, prevStart, prevEnd } = weekComparisonBounds(now)

  const currentDone = completed.filter(i => inCompletedWindow(i, curStart, endNow))
  const prevDone      = completed.filter(i => inCompletedWindow(i, prevStart, prevEnd))
