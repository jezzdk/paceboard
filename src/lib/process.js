...
export function processData({ mergedPRs, openPRs }, now = new Date()) {
  ...
  const kpis = {
    ...
    prsNoReviewer: openPRStats.filter(p => !p.draft && p.awaitingFirstReview).length,
  }
  ...
  const merged30d = mergedPRs.filter(
    p => new Date(p.merged_at) >= thirtyDaysAgo && new Date(p.merged_at) <= endNow
  )
  ...
  const histogram = ltBuckets.map((b, i) => ({
    label: b.label,
    count: lt30.filter(h => h > (ltBuckets[i - 1]?.max ?? 0) && h <= b.max).length,
  }))
