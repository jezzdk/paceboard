...
export default function App() {
  ...
  const [oauthStatus, setOauthStatus] = useState(() => {
    ...
    const isLinearCb =
      params.has("code") && !!sessionStorage.getItem("linear_oauth_state");
  ...
  const [linearOauthStatus, setLinearOauthStatus] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    return params.has("code") && !!sessionStorage.getItem("linear_oauth_state")
      ? "exchanging"
      : "idle";
  });
  ...
  const [pollInterval, setPollInterval] = useState(() =>
    parseFloat(localStorage.getItem(LS_POLL_INTERVAL) || "1"),
  );
  const [linearToken, setLinearToken] = useState(
    () => localStorage.getItem(LS_LINEAR_TOKEN) || "",
  );
  const [linearTeam, setLinearTeam] = useState(
    () => localStorage.getItem(LS_LINEAR_TEAM) || "",
  );
  ...
  useEffect(() => {
    ...
    oauth.handleCallback().then((res) => {
      ...
      ghAll(
        ...
        .then((r) => {
          setAllRepos(r.sort((a, b) => a.full_name.localeCompare(b.full_name)));
          setStep("repos");
        })
        .catch(() => {
          setOauthStatus("error");
          setOauthError("Could not load repositories — please try again.");
        });
    });
  ...
  const run = useCallback(async () => {
    ...
    const since = new Date(
      Date.now() - GITHUB_LOOKBACK_DAYS * 86_400_000,
    ).toISOString();
  ...
  function onSettingsSave({
    ...
    linearToken: newLinToken,
    linearTeam: newLinTeam,
  }) {
  ...
  function onDisconnect() {
    ...
    [LS_TOKEN, LS_REPOS, LS_REPO, LS_LINEAR_TOKEN, LS_LINEAR_TEAM].forEach(
  ...
  const countdownSec =
    nextAutoAt != null
      ? Math.max(0, Math.ceil((nextAutoAt - Date.now()) / 1000))
      : null;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="border-b px-6 py-3 flex items-center gap-4 flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          {linearToken && (
            <Badge
              variant="outline"
              className="text-[10px] font-mono text-violet-500 border-violet-500/40"
            >
              Linear
            </Badge>
          )}
      ...
      {result && !loading && (
        <DashboardPage result={result} linear={linearResult} />
      )}
          ...
          {lastFetch ? (
            <>
              ...
              <span className="font-medium text-foreground/80">
                {lastFetch.toLocaleDateString(undefined, {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}{" "}
