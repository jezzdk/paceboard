...
export function KpiStrip({ kpis, fmtH, fmt, owner, repo }) {
  const base =
    owner && repo ? `https://github.com/${owner}/${repo}/pulls` : null;

  const items = [
    {
      label: "Merged PRs",
      value: fmt(kpis.mergedCount),
      tip: "Total pull requests merged within the selected period.",
      href: base ? `${base}?q=is%3Apr+is%3Amerged` : null,
    },
    {
      label: "Open PRs",
      value: fmt(kpis.openCount),
      warn: kpis.openCount > 10,
    ...
    {
      label: "Stale PRs (>3d)",
      value: fmt(kpis.stalePRs),
      warn: kpis.stalePRs > 3,
      tip: "Open (non-draft) PRs that have been waiting for more than 3 days without being merged or closed.",
      href: base ? `${base}?q=is%3Apr+is%3Aopen` : null,
    },
    {
      label: "No Reviewer",
      value: fmt(kpis.prsNoReviewer),
      danger: kpis.prsNoReviewer > 0,
