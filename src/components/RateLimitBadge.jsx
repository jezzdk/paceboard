...
export function RateLimitBadge({ rl }) {
  ...
  const resetIn = Math.max(
    0,
    Math.ceil((rl.reset * 1000 - Date.now()) / 60000),
  );
      ...
      <Tooltip>
        ...
        <TooltipContent className="max-w-[260px] space-y-1 py-2">
          ...
          <p className="text-xs leading-relaxed opacity-90">
            GitHub allows 5,000 authenticated API requests per hour. Paceboard
            uses these to fetch PR lists, details, reviews, and comments.
          </p>
          <p className="text-xs opacity-75">
            {rl.remaining.toLocaleString()} remaining · {resetStr}
          </p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
